...
This experiment was created using PsychoPy3 Experiment Builder (v2021.2.3),
    on Mon 15 Aug 2022 03:33:58 PM CEST
...
expName = 'test4'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/home/electrobrain/lighteeg/new_task_ARSQ/test4.py',
...
gridLine5 = visual.ShapeStim(
    win=win, name='gridLine5',
    size=(0.6, 0.6), vertices='triangle',
    ori=90.0, pos=(-0.3, 0),
    lineWidth=1.5,     colorSpace='rgb',  lineColor='white', fillColor='white',
...
i_eoecComponents = [key_resp_3, text_4, mouse_2]
...
gotValidClick = False  # until a click is received
# keep track of which components have finished
i_arsqComponents = [text_5, key_resp_4, mouse]
...
# set up handler to look after randomisation of conditions etc
nb_taskloop = data.TrialHandler(nReps=5.0, method='random', 
    extraInfo=expInfo, originPath=-1,
...
for thisNb_taskloop in nb_taskloop:
    ...
    if condsFile == "nBack1Cond.xlsx":
        ...
        instrText.text = "This is the end of 2-back trials. You are about to start 3-back trials. This means that instead of pressing ‘space’ whenever the square appears in the same position as on the position on two trials before, you are required to press ‘space’ whenever the square appears in the same position as on the position three trials before. For example if the square appeared in left down corner on trial 1, you should press ‘space’ if the square appears in the left down corner on trial 4. Press ‘s’ to continue."
    # keep track of which components have finished
    i_nbackComponents = [instrTxt, instrResp, instrImg]
    ...
    while continueRoutine:
        ...
        if instrResp.status == STARTED and not waitOnFlip:
            theseKeys = instrResp.getKeys(keyList=['c'], waitRelease=False)
    ...
    for thisComponent in i_nbackComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    nb_taskloop.addData('instrTxt.started', instrTxt.tStartRefresh)
    nb_taskloop.addData('instrTxt.stopped', instrTxt.tStopRefresh)
    nb_taskloop.addData('instrImg.started', instrImg.tStartRefresh)
    nb_taskloop.addData('instrImg.stopped', instrImg.tStopRefresh)
    skip_this_routine=False
    
    if instrTxt.keys is not None:
        if 's' == instrTxt.keys:
            skip_this_routine=True
    ...
    # set up handler to look after randomisation of conditions etc
    nb_trials = data.TrialHandler(nReps=5.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    # completed 5.0 repeats of 'nb_trials'
    
    thisExp.nextEntry()
    
# completed 5.0 repeats of 'nb_taskloop'
