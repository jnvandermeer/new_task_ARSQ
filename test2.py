...
This experiment was created using PsychoPy3 Experiment Builder (v2021.2.3),
    on Mon 15 Aug 2022 02:16:46 PM CEST
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/home/electrobrain/lighteeg/new_task_ARSQ/test2.py',
...
slider = visual.Slider(win=win, name='slider',
    ...
    labels=('-- ','-','0','+','++'), ticks=(1, 2, 3, 4, 5), granularity=1.0,
...
# Initialize components for Routine "reset_lpt"
reset_lptClock = core.Clock()
...
InstructionComponents = [key_resp_3, text_4, mouse_2]
...
while continueRoutine:
    ...
    if key_resp_3.status == STARTED and not waitOnFlip:
        theseKeys = key_resp_3.getKeys(keyList=['y', 'n', 'left', 'right', 'space', 'c'], waitRelease=False)
...
wakeup = data.TrialHandler(nReps=3.0, method='random', 
...
for thisWakeup in wakeup:
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    whiteClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "white"-------
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = whiteClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=whiteClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        ...
        if img_whte.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > img_whte.tStartRefresh + 0.25-frameTolerance:
                # keep track of stop time/frame for later
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if img_black.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > img_black.tStartRefresh + 0.25-frameTolerance:
                # keep track of stop time/frame for later
...
# completed 3.0 repeats of 'wakeup'
...
gotValidClick = False  # until a click is received
# keep track of which components have finished
instrComponents = [text_5, key_resp_4, mouse]
...
while continueRoutine:
    ...
    if key_resp_4.status == STARTED and not waitOnFlip:
        theseKeys = key_resp_4.getKeys(keyList=['y', 'n', 'left', 'right', 'space', 'c'], waitRelease=False)
...
for thisTrial in trials:
    ...
    # ------Prepare to start Routine "reset_lpt"-------
    continueRoutine = True
    ...
    # keep track of which components have finished
    reset_lptComponents = [image]
    for thisComponent in reset_lptComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    reset_lptClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "reset_lpt"-------
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = reset_lptClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=reset_lptClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
